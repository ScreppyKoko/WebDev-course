...
// \Наприме, он сработает и так Math.max(22,33,1,2,33,44,55,1,100,45) и так Math.max(33,1,2) (проверьте самостоятельно). 
// Минус метода - его тяжело использовать если мы не знаем количество передаваемых в него аргументов. 
// Давайте напишем следующий код (см. код). Как видите, теперь не важно сколько элементов в массиве, и мы всегда сможем найти максимум. 
// Обратите внимание, в качестве контекста мы передали null. 

const out8 = document.querySelector('.out-8');
let ar8 = [22, 33, 1, 2, 33, 44, 55, 1, 100, 45];

out8.textContent = Math.max.apply(null, ar8);

// Task 9
// По нажатию на кнопку b-9 запускается анонимная функция, которая через apply передает функции t9 контекст out-9 и массив ar9 элементов. 
// Функция t9 должна вывести максимальный элемент массива.
...
// Task 10
// Допишите анонимную стрелочную функцию внутри some такую, что принимает элементы массива ar9 из some 
// и возвращет true если тип данных элемента boolean.
